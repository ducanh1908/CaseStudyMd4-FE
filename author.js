...
`
    ...
    <td> <a href="${data.linkWiki}">Detail</a></td>
    <td>
...
function getQuantity(){
            $('#quantityinput').change(function(){
                ...
                let record = $('#quantityinput option:selected').val();
      ...
      task
        ...
        .then(url => {
          ...
          $.ajax({
            ...
            <td><a href="javascript:void(0);" class="text-body font-weight-semibold">${data.name}</a></td>
            <td>${data.yearOfBirth}</td>
            <td>${data.worksAmount}</td>
            <td>${data.nationality}</td>
            <td>${data.linkWiki}</td>
